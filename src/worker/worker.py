...
class WorkerNode:
    ...
    def _get_gpu_info(self) -> bool:
        """Checks if an NVIDIA GPU is available and accessible."""
        try:
            import subprocess
            # Lightweight check: just see if nvidia-smi can list a GPU
            result = subprocess.run(['nvidia-smi', '-L'], capture_output=True, text=True, timeout=1)
    ...
    def _heartbeat_loop(self):
        ...
        while self._running:
            try:
                ...
                gpu_available = self._get_gpu_info()
                
                self.comm_client.send_heartbeat(
                    self.worker_id, 
                    load, 
                    cpu_usage, 
                    memory_usage,
