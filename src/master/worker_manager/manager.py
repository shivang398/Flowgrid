...
class WorkerManager(WorkerManagerInterface):
    ...
    def update_worker_status(self, worker_id: str, load: int, cpu_usage: float = 0.0, memory_usage: float = 0.0, gpu_available: bool = False) -> Worker:
        """
        ...
        with self._lock:
            ...
            worker = self._workers[worker_id]
            worker.update_heartbeat(load=load, cpu_usage=cpu_usage, memory_usage=memory_usage, gpu_available=gpu_available)
            return worker
